...
## §101（2026-09-14）进站加载页 —— CRT 灰屏 / 像素→清晰 / 真实进度预取

用户：进入网站前放一段加载动画 —— 老式 CRT 风格、灰色背景、中间大字 Loading、带进度条；**Loading 与进度条初始为像素风格，随加载进度逐渐变清晰**（像素块变小 → 锐利）；动画期间预载大部分网页/笔记/图片/动效资源。追加：四角要有老式显示器那种枕型畸变（屏幕几何）。

**出现时机**：仅 **首页 `/` 的整页加载**播放，且**本次会话只播一次**（sessionStorage）—— 会话内刷新首页、从内页返回首页、直接进入 `/photos/xxx` 等内页均不播。理由：站内已是客户端路由（§100），切页本就不重载；加载页的价值集中在「冷启动一次把站预热完」，重复播只会变成挡路的墙。

**动效核心 = Canvas 分辨率阶梯**（非模糊、非交叉淡化）：离屏 canvas 用极低内部分辨率（起步 ~24px 高）绘制 LOADING 与进度条 → 以 `imageSmoothingEnabled=false` 放大上屏（大色块）→ 随进度**逐档抬升内部分辨率**，像素块越来越小，末档与屏幕原生分辨率重合 = 完全锐利。前 70% 进度完成大部分爬升、后 30% 收尾，避免结尾突然锐化。

**屏幕几何（CRT 管面）**：整帧再经**分条带变形**（逐 y 条带水平缩放 + 偏移，弯曲量 ∝ 距屏幕中心距离²），四角内收（枕形）与边缘外凸（桶形）由**曲率参数符号**切换 —— 老式显示器严格说是桶形，用户描述为枕型，两者在实现里等价可调，dev 阶段给用户对比选定。叠加：四角压暗渐深暗角 + 1px 扫描线（~8% 透明）+ 极轻闪烁（reduced-motion 关闭）+ 可选显示器外框（默认加，略深灰一圈，把弯曲管面框住）。文字带极轻红/青边缘色差（呼应 §31 棱光语言，不另起配色）。

**视觉**：中灰底径向渐变（中心亮、四周暗）+ 大写 **LOADING**（站内 Maple 字体、等宽、宽字距）+ 分段格子进度条 + 右侧百分比。结束：短暂白闪（消磁感）→ 灰幕内收 → 移除。

**预取（真实进度，非假条）**：全部 14 路由 HTML（179 KB）+ 13 张展示 webp（689 KB）+ 字体子集（1052 KB）≈ **1.9 MB**；并发 4–6，`fetch(priority:'low')`，按 **已下载字节数**驱动进度条；`saveData`/2G 降级为仅预取 HTML（与 §100 的 Astro 预取策略一致）；首页玻璃球 island 不重复预取（本就 idle 后台加载）；图片 URL 由构建期与页面同一套 `getImage()` 参数取得，保证与页面引用完全同源。

**兜底（优先级高于效果）**：单资源失败计入完成不卡住；**总时长上限 ~6s**（到点无论进度平滑补满并放行）；最小显示 ~1.2s（保证像素→清晰看得出来）；无 JS → `<noscript>` 隐藏（页面正常）；`prefers-reduced-motion` → 静态锐利版 1s 后淡出，无像素动画/闪烁/变形动画；纯 CSS 强制淡出兜底（脚本异常不残留灰屏）。

**落点**：新增 `src/components/Preloader.astro`，**自包含**（内联 `<style is:inline>` + 内联 `<script is:inline>` + `<noscript>`）—— 首屏关键 CSS/JS 必须内联，否则先闪一帧无样式或白屏；仅在首页渲染，作为 `<body>` 首个元素（首帧即加载页），z-index 高于一切（Nav z60），`position: fixed` 不参与布局；结束移除画布与 DOM（含 body 滚动锁）。Base/其余页面零影响。

**性能边界**：画布 DPR ≤1.5（同 §69）、变形帧率封顶 30fps、仅加载页活动期运行；不新增任何运行时依赖（Canvas 2D + 已有字体）。

**验证**：`npm run build` 0 警告；无头探针断言（首帧存在加载页 / 同会话二次加载不出现 / 内页直接进入不出现 / 预取请求数与并发 / 结束后 DOM 与画布移除 / 无 JS 与 reduced-motion 分支可用）；限速走查真实进度推进与 6s 上限放行。

**已知取舍**：加载页本身**推迟了首页首屏**（与 §78/§90 的提速方向相反），以「一次性仪式感 + 全站预热」交换 —— 故严格限定首页 + 每会话一次；上限 6s 是硬约束，宁可进度不满也放行。

**实现记录（2026-09-14）** —— 三处只有动手才知道的事：

1. **内联 `<style>` 必须排在标记与脚本之前**：脚本要量覆盖层尺寸，若样式还没被解析，`canvas.clientWidth` 返回的是 canvas **默认内在宽度 300px**（不是 0，所以 `|| innerWidth` 兜底也不触发）→ 背板被定死 300×150 再拉伸显示，像素阶梯与变形全糊。现在尺寸以覆盖层自身为准，样式置顶。
2. **像素爬升要按「块尺寸」线性收窄，不能按分辨率线性爬升**：块大小 = H/res，res 线性涨 → 块按 1/res 掉，p=0.18 时就从 26px 掉到 2.5px，大色块阶段等于不存在（实测截图才发现）。改成块尺寸 26px → 1px 线性收窄，「变清晰」整段都看得见（内部高度实测 37 → 68 → 800）。
3. **顺手抓到真 bug：照片一直不是响应式的** —— Astro 5 的 `getImage()` 档位在 `srcSet.values`，而两处调用点读的是 `attributes.srcset`（不存在，恒 undefined）：13 张 webp 有 10 张是构建出来没人引用的死文件，各设备都下同一档。已修（并抽出 `src/lib/photo-images.ts` 让页面与预取清单同源）—— **预取清单的 689KB 图片之所以成立，正是因为这次修好**。

4. **客户端路由会把加载页「换」到别的页面上（已修）**：用户报「点导航栏站名前会出现无动画、字体变小的加载页」。复现：换页时 Astro 把目标页 body 换进来，**换入的内联脚本不执行**，但 `#pl` 标记照常显示 → 屏幕上留下灰屏 + DOM 兜底 LOADING 文字，等 CSS 兜底 8.5s 才淡出（`drawn:false` 证实渲染器没跑）。**修法：组件不渲染任何可见标记，整套 DOM 交给内联脚本创建** —— 换页时没有标记可显示，脚本也不会执行，双保险；再加 `document.readyState !== 'loading'` 判断（真·首次解析才播，换页副本此时文档已 complete）。

**时长语义（用户二次定稿）**：**最短 10s**，10s 到点若还没下完就**停在 99%** 直到全部到手 —— 不再有原先的 6s 强制完成。实现：显示进度 = 时间轴（10s smoothstep，像素→清晰铺满全程），未下完时封顶 99%（`min(timeP, .99)`）；真实字节进度照常统计（`__plState`/estimate/done），只是不再压住显示。防线改成两道：**idle 超时**（单请求 10s 没收到字节即算完，慢但在动的下载不会中招）+ **60s 逃生口**（网络僵死时唯一出路，可设 Infinity 关掉）。

**视觉二稿（同日，用户逐项反馈后重做）**：① 时长 10s → **5s**；② **治「字体与进度条抽搐」** —— 根因是内部分辨率逐帧连续变化，字号/字距每帧被 `Math.round` 重新量化 → 改**档位阶梯** `BLOCKS`（26→1 共 13 档，同档内完全静止，跳档即一次干净的分辨率提升）；③ 扫描线由 1px/3px 加粗到 **2px/4px** 并整体缓慢上下漂移（`.pl-scan` + `pl-drift`），另加 CRT 回扫亮带（`.pl-roll` 每 7.5s 扫过）；④ 暗角由单圈改**多段渐变**（40%→58%→74%→87%→95%→100% 逐级加深）+ 顶部玻璃高光 + 管面中央微亮（`.pl-vig`）；⑤ **立体化**：管面内缩 `INSET` 0.98→0.94、外壳色改深（`#242424`），管面（`#8c8c8c`）嵌在机壳里；⑥ **荧光柔光**：画面缩到 1/4 模糊后以 `lighter` 叠回（`applyBloom`，α0.26）模拟显像管光晕；⑦ **左下角 1-bit 像素终端字**（`drawTerm`：固定小画布绘制→alpha 阈值化→最近邻贴到屏幕空间，避开主分辨率抽搐；先画一遍供柔光、bloom 后再画一遍保锐利），内容带真实数据（缓存项数/已下载 MB/SIGNAL 状态）。

验证：无头探针 14/14 —— 首帧存在 / 会话内不重播 / 内页不出现 / 无 JS 无物可显 / 时间采样下内部高度 31→71→800 / 四角内收 / 画布尺寸 = 覆盖层×min(DPR,1.5) / 预取 33 项全部发出、1.88MB 跑完 9.94s 后自动摘 DOM 恢复滚动 / 资源卡死 12s 时停在 99.0% 不放行、僵死请求最终仍会放行（59.95s）/ reduced-motion 直接原生分辨率；另有点站名回归探针（同页与内页两路均不再出现加载页）。

## §102（2026-09-16）加载页改瑞士版 + 玻璃球入场过渡

用户：加载页整版换成**新瑞士 / 国际主义排版**，冷静理性、规整分栏网格、清晰信息层级；并明确「此加载页整体使用 Helvetica Bold 来仿新瑞士风格」。此后逐项追加：网格构建动画、酸性装饰、噪点随加载由糊到清、字号层级、数据标签贴右、`///LOADING`、开场解密揭示，最后是**入场过渡**（呼出球 → 褪光学 → 撤场 → 首页）。§101 的 CRT 那套（BLOCKS 档位阶梯 / blit / drawTerm / applyBloom / 枕形条带）**整套移除**。

**版式**：`///LOADING` 眉标 + 巨数百分比（`22vh`）+ 右侧四行数据（ROUTES/ASSETS/BYTES/FONTS，标签与数值都右对齐、贴住版心右缘）+ 四角小标签 + 12 栏网格 + 上下横线 + 全宽彩虹加载条。**除百分比外一律 Maple**；百分比留 Helvetica —— 它是视觉主角，字形骨架不换。

**开场四件事**（都随进度收敛）：① 网格自左向右/自上而下构建；② **缓入模糊** `1−u^POW`（开头几乎不动、末尾猛收）；③ 灰雾散尽；④ 噪点纹。后四项的历史与取舍见下。

**字体**：文字改 Maple 后，标签/眉标/数据栏走 `MaplePre` —— 一个**构建期内联**的子集（ASCII + 常用符号，取 **Bold 原件**切）。取 Medium 会被合成加粗（伪粗），与 §98 的真粗体不是一回事。10 个 slider 那种「调完再定」的参数现在都是**命名常数**，改一行即可。

**解密揭示（单次）**：除百分比外的文字逐字从乱码**锁定**成真字（左→右），元素间依次起步。乱码字符集**必须落在 MaplePre 子集内**（`A-Z 0-9 /.:%()-+`）—— 用子集外的字，动画途中会逐字回退到别的字体，字形当场变样。乱码字**每 `FLICKER_MS`(100ms) 才翻一次**，不是每帧 —— 逐帧是 60 次/秒，糊成噪点、看不出「字在跳」。用「位置 + 时间刻度」的哈希代替 `Math.random`：同刻度内稳定、跨刻度才翻新，不必给每个字符存状态。
> **虚拟时钟**：`v = max(真实耗时, 进度/0.9 × 总时长)`。前半项 = 用户定的时长（加载够久就按它跑完，手感保留）；后半项是硬保证 —— 进度到 1 时 v 必到终点，揭示**一定在交班前定格**。（交班条件本身要求 `shown ≥ 0.995`，两者天然同步。）**别改回「预估加载时长、按需加速」那版**：真正的瓶颈是等玻璃球就绪（`BALL_WAIT_MS` 最多 6s），从资源完成度上根本看不出来，估出的期限一收紧就是猛跳，实测照样有 5 个元素被切。

**入场过渡（与预览页 `dev-ball.astro` 同源）**：`呼出球(1.7s) → 褪光学(0.3s) → 撤场 → 进入首页`。

- **加载页的球与首页的球是两个独立 mesh**（`FluidGlass` 的 `LoadingBall`）。以前是一个 mesh 从大球缩成小球再长回来 —— 加载页的状态会漏到首页那颗上：首页这颗原来读 `__lensOptics`，加载页褪光学留下的 `0` 会跟着漏过来、把首页的球变成一块平板玻璃。分家后首页这颗**一个 `__lens*` 都不读**，只认 `LENS_PROPS.scale` 与自己的入场量 `__homeGrow`。
- **洞开在底板上**（`alphaMap` 圆形挖洞），不是开在 DOM 加载层上。旧的 `pl-open` / `--pl-r` / `pl-rim`（一圈 `backdrop-filter` 假折射边环）整套删除 —— DOM 在 WebGL 之上、球的 shader 碰不到它，那圈边环本来就只是近似。现在加载页**真的被折到了**，因为它进了 3D 场景（`makePlate` 烘成 canvas → `LoadingPlate` z=5 的平面）。
- **球一褪完光学就整颗删掉**，没有收回动画；底板另走淡出（不能硬删，会跳）。
- 参数：球半径 **663px** / 贴脸距 **0.2**（球心随尺寸后推，别把相机吞进去）/ 洞径 **14×球半径**。

**⚠⚠ 出球的前提是「加载真的完成」**：`transition()` 只由 `drive()` 在 `资源全下完 && !holdingForBall && el_ms ≥ MIN_MS && shown ≥ 0.995` 时调用。**别把它挪到别处、别放宽那个条件** —— 那会让球在还没下完时就冒出来。实测：进度到 1 与球开始出现是同一时刻。

**实现记录 —— 只有动手才知道的事**：

1. **`makePlate` 的几何与文字一律从活着的 DOM 上量**，不再手写百分比与魔法系数。早期按 CSS 百分比在 canvas 里重算（`baseY = boxTop + 0.625*numSize` 之类），实测三块文字**各错各的**：顶部标签高 3px、`///LOADING` 低 1px、大数字高 11px。根因是 canvas 与 CSS 本是三套独立度量（行高 `.71` / flex 基线对齐 / `textBaseline`），手算的值只要 CSS 一动就重新失配 —— `clamp()` 那次也是同一个病。改成量 DOM 后 CSS 随便改都不会错（基线 = 行盒顶 + 半行距 + 上升部；`asc/desc` 取 canvas 的字体度量，浏览器给 DOM 排版用的就是同一套表）。
2. **文字对位探针**：`A=DOM 截图 / B=canvas 直铺 / C=WebGL 底板` 三方差分。实测 **B↔C 逐像素相同**（3D 平面无辜），差异全在 A↔B 的 canvas 排版上。⚠ 量之前必须**等解密跑完再截图**（它是实时动效，中途截会抓到两个不同的乱码帧，虚高到 0.6），且**顺序是先 park 再等**（park 会 reset + 重开解密）。
3. **数据栏末行基线对齐**：用脚本量的 `--data-lift` 抬到大数字那条线上。差值**既不正比 vh 也不正比字号**（720 高差 7.22px / vh 1.00%、1080 高差 14.34px / vh 1.33%），因为 `.pl-hero` 里还叠着一根 16px 的固定结构行。⚠ 测量前必须先把 `--data-lift` **归零** —— 本函数会被 resize / `fonts.ready` 反复调用，不归零会量到「抬过之后的残差」，每调一次再抬一截，滚雪球。
4. **模糊终点不是 0，是「原 48% 那一档」**（≈0.83px），且**必须烘进底板** —— 否则撤下 DOM 那一刻整页「咔」地变清晰。同理 `transition()` 里那句 `scene.style.filter='none'` 必须删掉。代价：`blur(≠0)` 会关掉次像素抗锯齿，末段文字比 `filter:none` 略软（这正是要的效果）。
5. **`__lensBall` 与 `__homeGrow` 必须同时置零**：首页那颗的可见性只看前者，置零晚一步它就会以满尺寸闪一下。
6. **撤场时洞不跟着收** —— 收了的话底板上「没被挖掉」的面积会重新长回来，等于加载页在淡出途中又冒回来一下（左下角百分比闪一下）。
7. **模板字符串陷阱（栽了两次）**：`bootCss` / `bootJs` 本身就是 JS 模板字符串，**注释里出现反引号会把它提前闭合**，后面的 CSS 被当成 JS 求值 —— 报错位置与真凶毫无关系（注释里写 `.pl-stage` 换来一句 `stage is not defined`）。`${` 同理。`<style is:inline>` 的内容也**不插值 `${}`**（写成 `${fontB64}` 会原样输出字面量 → CSS 解析失败 → 字体静默回退）。
8. **重复调用型函数的终值要从稳定源取**：`startDecrypt()` 的「终值」取自节点当前值，而它会被滑块/重播反复调用 —— 上一轮中途再调一次，抓到的就是乱码，那个乱码被当成终值、文字再也回不到真值。修法是进函数先 `finishDecrypt()`。
9. **`paintCounts()` 的补写**：解密期间数值列被让出去（`decOn` 早退），而它平时只在 `drive()` 里跑 —— `transition()` 把 `ended` 一置位 `drive()` 就 return，不在这里补一次，数值列会永远停在标记里的初始值 `00 / 00` 并被烘进底板。

**预览页 `/dev-ball/`**：临时迭代用（带速度/模糊起/曲线/字距/解密/闪动/光学/球半径/贴脸距/洞径 10 个滑块 + 空格播放暂停 + 进度条），上线时一并推送但加了 `noindex`；验收完整个文件删除。

**验证**：`npm run build` 15 页 0 警告；路由 curl 全 200、`/nope-xyz` 404；文字对位 @1920 四条带全 0px、逐像素差 **0.067/255**（@1280 三条带 0px，大数字 1px —— 亚像素取整，1920 下同元素残差 0.4）；出球时机两轮实测严格晚于进度到 100%；窄屏 800 走 `done()` 淡出而非 `transition()`；首页冒烟零错误、`__lensScale`/`__lensOptics` 已交还。
