...
验证：无头探针 13/13 —— 首帧存在 / 会话内不重播 / 内页不出现 / 无 JS 隐藏 / 内部高度单调 37→68→800 / 四角内收 / 画布尺寸 = 覆盖层×min(DPR,1.5) / 预取 33 项全部发出且 1.88MB 跑完后自动摘 DOM 恢复滚动 / 资源卡死时 6.36s 上限放行 / reduced-motion 直接原生分辨率。
