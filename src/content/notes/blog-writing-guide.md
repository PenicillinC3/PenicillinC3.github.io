...
- **放哪（按卷分文件夹）**：照片建议按所属卷建子文件夹（`src/content/photos/<卷slug>/`），同卷的 `xxx.jpg` 与 `xxx.md` 放一起。引用（`cover:` 与 `![[…]]`）**只写文件名、不含文件夹**，所以只要照片文件名全站唯一，怎么整理目录都不影响引用。
- **成卷**：新建 `src/content/series/<卷slug>.md`，frontmatter 里 `cover: <卷内照片slug>` 指定封面；给卷内照片 md 加上 `series: <卷slug>`，照片即自动按拍摄日期排进本卷。胶片画廊上显示的日期与地点字幕取的是**封面照片**的 `date`/`location`；卷 frontmatter 可选 `workName: 作品名`，填了画廊字幕优先显示它（缺省回落到地点）。
- 卷的标题、故事正文写在 series md 里，支持全部 Markdown 排版与同目录插图（横幅整行 / 竖幅左图右文靠图片自身比例自然排布）。照片**默认排在正文之后**，想让某张照片插进正文里就用下面的「图文交错」标记。
- 不挂卷的照片保留文件、不进胶片画廊。

### 图文交错：把照片插进正文里

卷正文不再是「文字在前、照片在后」的固定顺序 —— 在 series md 的正文里，**独占一行**写 `![[照片slug]]`（slug = 卷内照片的文件名），那张照片的卡片（图 + 它的标题与说明）就会**就地插在**这一行的位置：
...
- 标记里的 slug 必须属于本卷，写错会在 `npm run build` 时直接报错并点名（不会静默丢图）。
