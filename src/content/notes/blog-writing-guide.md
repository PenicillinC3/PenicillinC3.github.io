...
- **放哪（按卷分文件夹）**：照片按所属卷建子文件夹（`src/content/photos/<卷名>/`），同卷的 `xxx.jpg` 与 `xxx.md` 放一起。照片的「编号」= **文件夹名/文件名**（如 `nikon-roll/dsc0007`）——相机文件名跨卷常重名，所以引用时要带卷前缀；**同一卷内文件名不重复**即可。不同卷重名完全没问题。
- **成卷**：新建 `src/content/series/<卷slug>.md`，frontmatter 里 `cover: <卷名/照片编号>` 指定封面（如 `cover: nikon-roll/dsc0025`）；给卷内照片 md 加上 `series: <卷slug>`，照片即自动按拍摄日期排进本卷。胶片画廊上显示的日期与地点字幕取的是**封面照片**的 `date`/`location`；卷 frontmatter 可选 `workName: 作品名`，填了画廊字幕优先显示它（缺省回落到地点）。
- 卷的标题、故事正文写在 series md 里，支持全部 Markdown 排版与同目录插图（横幅整行 / 竖幅左图右文靠图片自身比例自然排布）。照片**默认排在正文之后**，想让某张照片插进正文里就用下面的「图文交错」标记。
- 不挂卷的照片保留文件、不进胶片画廊。

### 图文交错：把照片插进正文里

卷正文不再是「文字在前、照片在后」的固定顺序 —— 在 series md 的正文里，**独占一行**写 `![[卷名/照片编号]]`，那张照片的卡片（图 + 它的标题与说明）就会**就地插在**这一行的位置：
...
- 引用要带卷前缀（`卷文件夹名/文件名`，同 `cover:` 的写法）；漏写前缀会因找不到照片而构建报错点名。
- 想放几张、放哪都行，写几个标记插几张；**没被标记的照片**自动按拍摄日期补在正文后面。
- 标记引用的照片必须属于本卷，写错会在 `npm run build` 时直接报错并点名（不会静默丢图）。
- 一个标记都不写 = 保持旧版式（整段正文 + 底部照片区），已有的卷不受影响。
...
| `卷 xx 的正文引用了不属于本卷的照片：yy` | 正文 `![[yy]]` 标记的 slug 不在本卷内 | 用该卷内照片的文件名；或先给照片 md 加 `series: 卷slug` |
