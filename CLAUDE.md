...
- **页面形态**：
  - `/` 单屏欢迎页：中央华文中宋大站名 **PenicillinC3** + 右上**液态玻璃镜头**（spec §34：`LensGlass.tsx` = React+Three `MeshTransmissionMaterial` 全真折射岛，client:visible，折射画布内自绘背景而非 DOM；slot 定位/跟随调参入口见 spec；<900px 隐藏）。`.main:has(.welcome)` 特判居中；**页脚仅首页渲染**（Base isHome 判断；其余页无最下方栏），版权行 `© 2026 PenicillinC3 Via Claude Code`（无回到首页链接）。body 是 flex 纵向吸底布局。
...
8. 依赖上限纪律：`dependencies` = `astro`、`ogl`（射线着色器 §26）、`react18 + react-dom + three + @react-three/fiber + @react-three/drei + maath`（**仅首页液态玻璃镜头岛** spec §34；安装须 `--legacy-peer-deps`，drei 的 RN optional peer 会拉 @types/react@19 冲突；构建时首页 chunk 体积警告属预期）。devDependencies = typescript + `subset-font`（字体子集构建 §28/§29）。**禁止再加 UI 框架/运行时依赖**。字体：允许嵌入（font/ 原件 + public/fonts 子集；新文案后跑 `npm run fonts:subset`）。
