...
- **内容层**：`src/content.config.ts` 用 zod 定义 6 个集合 —— `notes`/`musings`/`links`/`projects`/`photos`/`series`。共享字段 `title/date(YYYY-MM-DD)/summary?/draft?`；`photos` 经 `image()` 引用**同目录同名文件**（md 与其图按卷分文件夹存放：`src/content/photos/<卷slug>/`，§95；集合用 glob loader + generateId 取**文件名**为 slug —— 引用只认文件名、不含文件夹，照片名需全站唯一），并带可选 `series` slug 挂靠；`series`（摄影卷）有 `cover`（卷内照片 slug，必填）。frontmatter 错误在构建期报错拦截，这是特性。
