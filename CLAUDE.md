...
- **页面形态**：
  ...
  - `/notes`、`/musings`、`/links` 列表为**单列通栏大卡**（spec §24/§25：一行一张 ~1100px；PostCard 标题↔日期对顶、摘要 76ch 限宽；三栏目卡统一 `min-height:230px` 对齐）+ `/[slug]` 详情；笔记列表有标签过滤（PostList 事件委托 + `.is-out` 原位淡出但**保留占位不塌缩**，spec §53 —— 页面高度/卡片位置全程不变）。
