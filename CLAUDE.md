...
- **进站加载页**（spec §101）：CRT 灰屏 + 大字 LOADING + 分段进度条，像素块随进度由 26px 收到 1px 变锐利，管面用逐行条带变形做枕形（四角内收）。**只在首页 `/` 的整页加载且本会话首次**播放（sessionStorage `plPlayed`；会话内刷新、从内页返回、直接进内页都不播）——唯一开关是 `Base.astro` 的 `{isHome && <Preloader />}`。`src/components/Preloader.astro` **必须自包含内联**（style + script + noscript）：模块脚本是 deferred 会先闪一帧，且**内联 `<style>` 必须在标记与脚本之前**（脚本要量视口尺寸；顺序错了脚本量到的是 canvas 默认 300×150，背板被定死）。调参旋钮：`BLOCK_BIG`（起步像素块）/ `K`（曲率正负切枕形桶形）/ `INSET` / `MIN_MS`·`MAX_MS`（1.2s / 6s 硬约束）/ `CONC`。预取清单在 `src/lib/prefetch-manifest.ts`（13 路由 + 13 展示图 + 7 字体 ≈1.9MB，按**已下载字节**驱动进度，省流量/2G 降级为仅 HTML）。六重兜底：单资源失败不卡、6s 上限、1.2s 下限、`<noscript>` 隐藏、reduced-motion 直接锐利版、CSS 8.5s 强制淡出（脚本全死也不会留灰屏）。
