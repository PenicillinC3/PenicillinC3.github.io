...
- **进站加载页**（spec §101）：CRT 灰屏 + 大字 LOADING + 分段进度条，像素块随进度由 26px 收到 1px 变锐利，管面用逐行条带变形做枕形（四角内收）。**只在首页 `/` 的整页加载且本会话首次**播放 —— 组件 `src/components/Preloader.astro` **只渲染一段内联脚本，整套 DOM 由脚本自己建**（⚠ **别改回「渲染标记 + 脚本增强」**：客户端路由换页会把首页 body 换进来，换入的内联脚本**不执行**，标记却会显示 → 留下无动画灰屏等 CSS 兜底淡出，§101 实测复现）；再加两道闸：`document.readyState !== 'loading'` 直接返回（换页副本此时文档已 complete）、sessionStorage `plPlayed` 一标签页一次。开关是 `Base.astro` 的 `{isHome && <Preloader />}`。**内联 `<style>` 必须在脚本之前**（脚本要量覆盖层尺寸；顺序错了量到的是 canvas 默认 300×150，背板被定死）。时长：**最短 5s**（时间轴驱动，像素→清晰铺满全程），**到点还没下完就停在 99%** 直到全部到手（真实字节进度照跑，只不再压住显示）；僵死请求靠 idle 超时（10s 无字节即算完）+ 60s 逃生口。视觉：管面嵌在深色机壳里（`INSET` 内缩 + `K` 曲率切枕形/桶形）+ 扫描线漂移与回扫亮带（`.pl-scan`/`.pl-roll`）+ 多段渐变暗角与顶部高光（`.pl-vig`）+ 荧光柔光（`applyBloom`，1/4 缩放模糊后 lighter 叠回）+ 左下角 1-bit 像素终端字（`drawTerm`，先画一遍进柔光、bloom 后再画一遍保锐利）。**分辨率走档位阶梯 `BLOCKS`**（同一档内完全静止，跳档才是提升）—— 别改回逐帧连续变，那样字号每帧被重新量化、画面会「抽搐」。预取清单在 `src/lib/prefetch-manifest.ts`（13 路由 + 13 展示图 + 7 字体 ≈1.9MB，省流量/2G 降级为仅 HTML）。兜底：单资源失败不卡、99% 停等 + 60s 逃生口、无 JS 无物可显（不渲染标记）、reduced-motion 直接锐利版、CSS 14s 强制淡出（脚本全死也不会留灰屏）。
