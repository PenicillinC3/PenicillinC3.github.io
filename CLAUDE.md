...
npm run build        # 产物到 dist/（验收标准：0 error / 0 warning，当前 15 页 = 14 正式 + /dev-ball 预览）
...
npm run fonts:subset # 新增内容后重建 woff2 子集（song §28 + maple §29 + maple-pre §102）
```
...
- **页面形态**：
  ...
  - **字体**（spec §28/§29/§32/§57）：正文 = `MapleMono Web`（子集；摄影页 page-dark 覆写为华文中宋）；代码 = `JetBrains Mono Web` 打头 + 字链兜底 —— `--font-mono` = JetBrains → 正体 `MapleMono Web`（§57：JetBrains 缺中文字形，代码内**非注释中文**逐字落正体 Maple，拉丁全归 JetBrains）；注释栈 `--font-comment` = JetBrains Italic → `MapleMono Italic Web` 子集 —— 英文注释 JetBrains 斜体、**注释中文逐字落斜体 Maple**（markdown 已切 prism 高亮，勿改回 shiki）。`npm run fonts:subset` 重建 song/maple/maple-italic/maple-ui/maple-pre（§102 加载页内联子集，ASCII+Bold 面）（§90 UI 极小号子集，置于 --font-sans 栈首。**首页提速关键**：框架文案全命中它、正文大写 238KB 子集不下载；其外汉字逐字回退到 MapleMono Web）六件（+ maple-bold §98：真粗体，仅 strong/b/th 等 ≥600 字重页面按需下载；Medium 面字重范围 100–599、Bold 面 600–900 —— 勿把 Medium 面写回 100 900，否则粗体字形永不被采用）。`--font-song` 首项 Song Web。导航栏 `.topbar` 显式 `font-family: var(--font-sans)`（spec §38：暗色页正文切宋体不影响导航，全栏目一致）。
  - 404 页有搜索框？没有 —— 搜索已全链路删除（fuse.js、SearchBox、/search 均不存在）。
- **进站加载页**（spec §102）：瑞士版（新瑞士/国际主义排版 + 酸性装饰 + 噪点），`///LOADING` 眉标 + `22vh` 巨数百分比 + 右侧四行数据 + 四角小标签 + 12 栏网格 + 上下横线 + 全宽彩虹加载条。**只在首页 `/` 的整页加载且本会话首次**播放 —— 组件 `src/components/Preloader.astro` **只渲染一段内联脚本，整套 DOM 由脚本自己建**（⚠ **别改回「渲染标记 + 脚本增强」**：客户端路由换页会把首页 body 换进来，换入的内联脚本**不执行**，标记却会显示 → 留下无动画灰屏等 CSS 兜底淡出）；再加两道闸：`document.readyState !== 'loading'` 直接返回、sessionStorage `plPlayed`。开关是 `Base.astro` 的 `{isHome && <Preloader />}`。**内联 `<style>` 必须在脚本之前**（脚本要量覆盖层尺寸，否则量到 canvas 默认 300×150）。⚠ **`bootCss`/`bootJs` 是模板字符串：注释里出现反引号或 `${` 会把模板提前闭合**，报错位置与真凶毫无关系（栽过两次）；`<style is:inline>` 的内容也不插值 `${}`。**字体**：标签/眉标/数据栏 = `MaplePre`（构建期内联子集，Bold 原件切，`readFileSync public/fonts/maple-pre.woff2` —— **新字体必须提交**否则线上构建失败）；百分比留 Helvetica。**开场**：网格构建 + **缓入模糊** `1−u^POW`（终点不是 0，是原 48% 那一档 ≈0.83px，**必须烘进底板**）+ 灰雾 + 噪点 + **解密揭示**（逐字乱码→锁定，单次；乱码字符集**必须落在 MaplePre 子集内**否则逐字回退；乱码每 100ms 才翻一次，逐帧会糊成噪点）。**`makePlate` 一律从活着的 DOM 上量**（基线 = 行盒顶 + 半行距 + 上升部），别手写百分比/魔法系数 —— 实测三块文字各错 3/1/11px；数据栏末行用 `--data-lift` 抬到大数字基线上，⚠ 测量前先归零否则滚雪球。**入场过渡**（`Preloader.transition()`，与 `/dev-ball/` 同源）：`呼出球(1.7s) → 褪光学(0.3s) → 撤场 → 首页`；**加载页的球与首页的球是两个独立 mesh**（`FluidGlass` 的 `LoadingBall`）—— 首页那颗一个 `__lens*` 都不读，只认 `LENS_PROPS.scale` 与 `__homeGrow`；**洞开在底板上**（alphaMap），不是 DOM 加载层（旧的 `pl-open`/`--pl-r`/`pl-rim` 已整套删除）；球褪完光学**整颗删掉、无收回动画**，底板另走淡出。⚠⚠ **出球前提是「加载真的完成」**：`transition()` 只由 `drive()` 在资源全下完且进度真到 100% 时调用，别挪别放宽。⚠ `__lensBall` 与 `__homeGrow` 必须同时置零（晚一步首页球闪一下）；撤场时**洞不跟着收**（收了加载页会冒回来）。参数：球半径 663px / 贴脸距 0.2 / 洞径 14×球半径 —— 都是命名常数。预取清单在 `src/lib/prefetch-manifest.ts`（13 路由 + 13 展示图 + 7 字体 ≈1.9MB，省流量/2G 降级为仅 HTML）。兜底：单资源失败不卡、到点没下完停 99%、idle 超时 10s + 60s 逃生口、无 JS 无物可显、reduced-motion 直接锐利版、CSS 18s 强制淡出。/dev-ball/ 是临时预览页（10 个调参滑块 + 空格播放），加 noindex 上线，验收后删。
...
2. **`<script is:inline>` / `<style is:inline>` 的内容都不插值**（`{expr}` 与 `${}` 一律按字面输出）—— 要注入得走 `set:html`。内嵌 JSON 用 `set:html={json}`，且先 `.replace(/</g,'\\u003c')` 防 `</script>` 逃逸（photos stage 的 roll-data 即此模式）。**更狠的一条**：`Preloader.astro` 的 `bootCss`/`bootJs` 本身就是模板字符串，**注释里出现反引号会把它提前闭合**，后面的 CSS 被当成 JS 求值 —— 报错位置与真凶毫无关系（栽过两次，spec §102）。
